...
fn do_not_overlap<'a>(claim1: &'a Claim) -> impl Fn(&'a Claim) -> bool {
    move |claim2| {
        ...
        if claim1 == claim2                  { true  }
        else if x1_start > x2_start + x2_dim { true  }
        else if y1_start > y2_start + y2_dim { true  }
        else if x2_start > x1_start + x1_dim { true  }
        else if y2_start > y1_start + y1_dim { true  }
        else                                 { false }
    }
}
